...
import bpjsRoutes from '@/routes/bpjs';
...
export default function BpjsEdit({ bpjs }: { bpjs: Bpjs }) {
    const { data, setData, put, processing, errors } = useForm({
        jenis_bpjs: bpjs.jenis_bpjs || '',
        perusahaan: bpjs.perusahaan?.toString() || '',
        karyawan: bpjs.karyawan?.toString() || '',
        total: bpjs.total?.toString() || '0',
    });
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        updateTotal();
        put(bpjsRoutes.update(bpjs.id).url);
    };

    return (
        <AppLayout breadcrumbs={[
            { title: 'Dashboard', href: '/dashboard' },
            { title: 'BPJS', href: bpjsRoutes.index().url },
            { title: 'Edit BPJS', href: '#' }
        ]}>
            <Head title={`Edit BPJS - ${bpjs.jenis_bpjs}`} />
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4 sm:p-6">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        ...
                        <p className="text-muted-foreground text-sm mt-1">Edit data BPJS: {bpjs.jenis_bpjs}</p>
                    </div>
                    <Link
                        href={bpjsRoutes.index().url}
                ...
                <div className="border rounded-2xl overflow-hidden bg-white dark:bg-gray-900 shadow-lg shadow-orange-100 dark:shadow-none">
                    <div className="bg-gradient-to-r from-orange-500 to-orange-600 dark:from-orange-700 dark:to-orange-800 px-6 py-4">
                        <h2 className="text-xl font-bold text-white">Informasi BPJS</h2>
                        <p className="text-orange-100 text-sm">Edit data BPJS dengan lengkap dan benar</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="p-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-2">
                                <label htmlFor="jenis_bpjs" className="block text-sm font-semibold text-gray-700 dark:text-gray-300">
                                    Jenis BPJS <span className="text-red-500">*</span>
                                </label>
                                <input
                                    type="text"
                                    id="jenis_bpjs"
                                    value={data.jenis_bpjs}
                                    onChange={(e) => setData('jenis_bpjs', e.target.value)}
                                ...
                                />
                                {errors.jenis_bpjs && (
                                    <p className="text-sm text-red-500">{errors.jenis_bpjs}</p>
                                )}
                            </div>
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-sm text-amber-700 dark:text-amber-400 mt-1">
                                        Perubahan data BPJS akan berdampak pada perhitungan gaji karyawan. Total persentase dihitung otomatis dari Perusahaan + Karyawan.
                                    </p>
                                </div>
                            </div>
                        </div>
