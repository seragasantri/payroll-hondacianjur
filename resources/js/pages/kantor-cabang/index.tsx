...
export default function KantorCabangIndex({ kantorCabang }: { kantorCabang: KantorCabangList }) {
    ...
    const hasActionAccess = isSuperAdmin || can('kantor-cabang.edit') || can('kantor-cabang.delete');
    ...
    const kantorCabangCreate = () => ({
        url: '/kantor-cabang/create',
    });

    const kantorCabangEdit = (id: number) => ({
        url: `/kantor-cabang/${id}/edit`,
    });

    const kantorCabangDestroy = (id: number) => ({
        url: `/kantor-cabang/${id}`,
    });

    const handlePerPageChange = (value: string) => {
        router.get(kantorCabangIndex().url, {
    ...
    const handleDelete = (kantorCabangId: number, kantorCabangName: string) => {
        Swal.fire({
            title: 'Hapus KantorCabang?',
            text: `Apakah Anda yakin ingin menghapus KantorCabang "${kantorCabangName}"?`,
            icon: 'warning',
            ...
            preConfirm: () => {
                return router.delete(kantorCabangDestroy(kantorCabangId).url, {
                    onSuccess: () => {
                        Swal.fire({
                            title: 'Terhapus!',
                            text: `KantorCabang "${kantorCabangName}" berhasil dihapus.`,
                            icon: 'success',
                            timer: 2000,
                    ...
                    onError: () => {
                        Swal.fire({
                            title: 'Gagal!',
                            text: 'Gagal menghapus KantorCabang.',
                            icon: 'error',
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Manajemen KantorCabang" />
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        <h1 className='text-3xl font-bold bg-gradient-to-r from-orange-600 to-orange-400 bg-clip-text text-transparent dark:from-orange-400 dark:to-orange-300'>
                            Manajemen KantorCabang
                        </h1>
                        <p className="text-muted-foreground text-sm mt-1">Kelola data KantorCabang dengan mudah</p>
                    </div>
                    <div className="flex items-center gap-3">

                        {(isSuperAdmin || can('kantor-cabang.create')) && (
                            <Link
                                href={kantorCabangCreate().url}
                ...
                <div className="border rounded-2xl overflow-hidden bg-white dark:bg-gray-900 shadow-lg shadow-orange-100 dark:shadow-none">
                    ...
                    <table className='w-full min-w-[800px]'>
                                ...
                                <th className='px-4 py-4'>
                                    <div className="relative">
                                        <input
                                            ...
                                            onChange={(e) => debouncedSearch('searchName', e.target.value, kantorCabangIndex().url)}
                                            placeholder='Cari KantorCabang...'
                                        />
                        ...
                        <tbody className='divide-y divide-gray-200 dark:divide-gray-800'>
                            {isSearching ? (
                            ...
                            ) : !kantorCabang?.data || kantorCabang.data.length === 0 ? (
                                <tr>
                                    <td colSpan={hasActionAccess ? 5 : 4} className="px-4 py-12 text-center">
                                ...
                                kantorCabang.data.map((item, index) => (
                                        ...
                                        {hasActionAccess && (
                                            <td className="px-4 py-3">
                                                <div className="flex items-center justify-center gap-1.5">
                                                    {(isSuperAdmin || can('kantor-cabang.edit')) && (
                                                        <Link
                                                            href={kantorCabangEdit(item.id).url}
                                                    ...
                                                    {(isSuperAdmin || can('kantor-cabang.delete')) && (
                                                        <button
                    ...
                    {kantorCabang?.meta && (
                        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-6 py-4 bg-orange-50/50 dark:bg-gray-800/30 border-t border-gray-200 dark:border-gray-800">
                            ...
                            <div className="text-sm text-gray-600 dark:text-gray-400">
                                Menampilkan <span className='font-bold text-orange-600 dark:text-orange-400'>{kantorCabang.meta.from}</span> sampai <span className='font-bold text-orange-600 dark:text-orange-400'>{kantorCabang.meta.to}</span> dari <span className='font-bold text-orange-600 dark:text-orange-400'>{kantorCabang.meta.total}</span> data
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                                <Link
                                    href={buildUrl(1)}
                                    className={kantorCabang.meta.current_page === 1
                                ...
                                <Link
                                    href={kantorCabang.links.prev ? buildUrl(kantorCabang.meta.current_page - 1) : '#'}
                                    className={!kantorCabang.links.prev
                                ...
                                {Array.from({ length: Math.min(kantorCabang.meta.last_page, 5) }, (_, i) => {
                                    let pageNum;
                                    if (kantorCabang.meta.last_page <= 5) {
                                        pageNum = i + 1;
                                    } else if (kantorCabang.meta.current_page <= 3) {
                                        pageNum = i + 1;
                                    } else if (kantorCabang.meta.current_page >= kantorCabang.meta.last_page - 2) {
                                        pageNum = kantorCabang.meta.last_page - 4 + i;
                                    } else {
                                        pageNum = kantorCabang.meta.current_page - 2 + i;
                                    }
                                    ...
                                    return (
                                        <Link
                                            ...
                                            className={`min-w-[2.5rem] h-10 flex items-center justify-center rounded-xl text-sm font-semibold transition-all duration-200 ${pageNum === kantorCabang.meta.current_page
                                ...
                                <Link
                                    href={kantorCabang.links.next ? buildUrl(kantorCabang.meta.current_page + 1) : '#'}
                                    className={!kantorCabang.links.next
                                        ? 'opacity-50 cursor-not-allowed bg-gray-200 dark:bg-gray-800 text-gray-500 dark:text-gray-500 rounded-xl px-3 py-2'
                                        : 'inline-flex items-center border-2 border-orange-200 dark:border-gray-700 rounded-xl px-3 py-2 text-gray-700 dark:text-gray-300 hover:bg-orange-500 hover:text-white hover:border-orange-500 dark:hover:bg-orange-600 dark:hover:border-orange-600 transition-all duration-200'
                                    }
                                >
                                ...
                                <Link
                                    href={buildUrl(kantorCabang.meta.last_page)}
                                    className={kantorCabang.meta.current_page === kantorCabang.meta.last_page
                                        ? 'opacity-50 cursor-not-allowed bg-gray-200 dark:bg-gray-800 text-gray-500 dark:text-gray-500 rounded-xl px-4 py-2 text-sm font-medium'
                                        : 'inline-flex items-center gap-1 border-2 border-orange-200 dark:border-gray-700 rounded-xl px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-orange-500 hover:text-white hover:border-orange-500 dark:hover:bg-orange-600 dark:hover:border-orange-600 transition-all duration-200'
                                    }
                                >
